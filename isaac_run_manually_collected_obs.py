...
def main():
    ...
    fabric_q = env.fabric_q[0].clone()
    fabric_qd = env.fabric_qd[0].clone()
    ...
    while True:
        ...
        manually_collected_obs = create_observation(
            ...
            fabric_q=fabric_q.detach().cpu().numpy(),
            fabric_qd=fabric_qd.detach().cpu().numpy(),
        ...
        obs = obs_dict["obs"]
        diff = (manually_collected_obs - obs).abs()
        print(diff)
        max_diff = diff.max()
